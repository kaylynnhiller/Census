...
NC <- NC %>% separate(NAME, sep = " ", into = c("County"))
...
NC %>%
  ...
  geom_sf(color = NA) + 
  scale_fill_continuous(labels=function(x) format(x, big.mark = ",", scientific = FALSE), name = "Total Females per County")

NC %>%
  ggplot(aes(x = estimate, y = reorder(County, estimate))) + 
  geom_point() +
  labs(x = "Total Females per County", y = "County") + 
...
census_api_key("0b702d86b97e9aa7cf3abb781d8b8b93e2c79825")
